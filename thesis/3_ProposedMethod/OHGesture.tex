...
Tương tự mô hình diffusion cơ bản bao gồm hai quá trình: quá trình tạo nhiễu (diffusion) $q$ và quá trình khử nhiễu (denoising process) $p_{\theta}$ với trọng số $\theta$. Tổng quan của mô hình đề xuất \textbf{OHGesture} được minh hoạ ở hình \autoref{fig:TrainingAndSampling}.
...
\begin{itemize}
	\item \textbf{Văn bản} (Text):
		...
		Vì vậy, trong công đoạn này đóng góp của luận văn là dựa trên dữ liệu tiếng nói có sẵn sử dụng Adobe Speech To Text để chuyển giọng nói thành văn bản, sử dụng Montreal Forced Aligner \cite{saxon2020robust} để căn chỉnh được thời gian tương ứng với số frame của cử chỉ để thu được các TextGrid. Trong TextGrid sẽ có thời gian tương ứng với từng từ, dựa trên thông tin này, FastText  \cite{bojanowski2017enriching} được  sử dụng để nhúng (embedding) văn bản thành các vector theo thời gian, với các vùng không có văn bản sẽ được cho là ma trận $\mathbf{0}$, các vùng có từ vựng tương ứng sẽ được gán là giá trị nhúng  theo từng khung hình của cử chỉ để được ma trận văn bản $\mathbf{v}^{1:M \times 300}$.
...
Với dữ liệu chính trong quá trình Diffusion chính là chuỗi cử chỉ $\bx_t \in \mathbb{R}^{1:M \times D}$, mô hình sẽ dùng một lớp tuyến tính (linear layer) để giảm kích thước chuỗi cử chỉ xuống còn $\mathbf{X} \in \mathbb{R}^{1:M \times 256}$. Quá trình này minh hoạ ở  \autoref{fig:OHGesture}.
...
Đầu tiên cử chỉ khởi tạo $\mathbf{S}$ và vector cảm xúc $\mathbf{E}$ được ghép lại với nhau để tạo thành một vectơ có kích thước $256$, bởi vì kích thước $256$ là kích thước vector tiềm ẩn. Sau đó được cộng thêm vector timestep $\mathbf{T}$ để tạo thành vector $\mathbf{Z} \in \mathbb{R}^{256}$. Vector $\mathbf{Z}$ thể hiện thông tin luận văn thực sự muốn điều khiển bằng diffusion có điều kiện.

\begin{equation}
	\mathbf{Z} = \mathbf{T}+ \operatorname{concat }(\mathbf{E}\ || \  \mathbf{S})
\end{equation}
...
Cụ thể, với ma trận tiềm ẩn $\mathbf{h}_t \in \mathbb{R}^{1:M \times 256}$ sau khi đi qua công đoạn kết hợp đặc trưng. Như minh hoạ ở \autoref{fig:OHGesture}, mô hình sẽ dùng một lớp tuyến tính (linear layer) để tăng kích thước chuỗi cử chỉ thành kích thước ban  đầu $\hat{\mathbf{x}}_{0} \in \mathbb{R}^{1:M \times D}$.
...
Điểm đặc biệt là có thể dựa trên việc học có điều kiện và không có điều kiện của classifier-free guidance \cite{ho2022classifier}, có thể nội suy giữa hai cảm xúc $\mathbf{e}_1$ và cảm xúc $\mathbf{e}_2$ khác nhau bằng cách cho điều kiện $c = \left[\mathbf{s}, \mathbf{e}_{1}, \mathbf{a}, \mathbf{v} \right]$ và $c_\varnothing = \left[\mathbf{s}, \mathbf{e}_{2}, \mathbf{a}, \mathbf{v} \right]$. Khi đó ta có thể viết lại $\hat{x}_{0 \gamma, c_{1}, c_{2}}=\gamma G \left(x_{t}, t, c_{1} \right)+(1-\gamma) G \left(x_{t}, t, c_{2}\right)$.
