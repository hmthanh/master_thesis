...
%như hình \autoref{fig:OHGesture}
...
%sau khi kết hợp tất cả các đặc trưng như âm thanh, cử chỉ khởi tạo, cảm xúc và văn bản thành một ma trận biểu thị thông tin của một frame. Luận văn sẽ dùng một lớp tuyến tính ($\operatorname{Linear}_{\theta}$) để giảm kích thước chuỗi cử chỉ xuống còn $\mathbf{h} \in \mathbb{R}^{1:M \times 256}$. Quá trình này minh hoạ ở lớp Cross-Local Attention  

\subsection{Công đoạn kết hợp đặc trưng}
%Áp dụng cơ chế Attention cho bài toán sinh cử chỉ
...
%, vì vậy phương pháp của luận văn là biểu diễn các đặc trưng của từng khung hình bằng các vector đặc trưng riêng lẻ, và với từng đặc trưng của khung hình riêng lẻ, luận văn mong muốn tìm được sự tương quan và mối quan hệ giữa các đặc trưng với nhau.
...
\begin{equation}
	...
	\mathbf{z} = \operatorname{concat }(\mathbf{E}\ || \  \mathbf{S}) + \mathbf{T}
\end{equation}
...
\begin{equation}
	...
	\mathbf{H} = \operatorname{concat}( \mathbf{Z}\  || \   \operatorname{concat}(\mathbf{X}\ || \  (\mathbf{V} + \mathbf{A}) ) )
\end{equation}

Sau khi có ma trận $\mathbf{H} \in \mathbb{R}^{1:M \times P}$ theo \autoref{eq:FrameConcat} thể hiện đăng trưng theo khung hình từ frame $1$ đến frame $M$, với mỗi frame có kích thước $P$ là tổng của các đặc trưng đã concat. Với $\mathbf{X} \in \mathbb{R}^{1:M \times 256}$, $\mathbf{Z} \in \mathbb{R}^{1:M \times 256}$ và $\mathbf{A}, \mathbf{V} \in \mathbb{R}^{1:M \times 64}$, thì $P$ sẽ có kích thước $256 * 2 + 64$.
...
Ma trận $\mathbf{H}$ sẽ tiếp tục đi qua lớp Cross-Local Attention để tính tương quan giữa các đặc trưng cục bộ: 

\begin{equation}
	\mathbf{h}^{1:M}_{t}  = \operatorname{Linear}_{\theta}  ( \operatorname{Cross-Local\ Attention}(H) )
	\label{eq:CrossLocalAttention}
\end{equation}

%Sau đó để căn chỉnh theo miền thời gian các đặc trưng về cảm xúc và cử chỉ khởi tạo luận văn sẽ copy vector $\mathbf{Z}$ để các bản sao của nó được ghép thành một chuỗi vector đặc trưng tương đồng với $M$ khung hình. Sau đó đưa vào lớp local attention (chú ý cục bộ). 

Tương tự ý tưởng từ phương pháp Routing Transformer \cite{roy2021efficient}, Cross-local Attention cho thấy rằng sự quan trọng trong việc xây dựng biểu diễn các vector đặc trưng trung gian trước khi đi qua lớp Transformer Encoder như \autoref{fig:OHGesture}. Các vector đặc trưng sẽ được cộng thêm một vector sinusoids hay mã hoá vị trí tương đối (Relative position encoding \cite{vaswani2017attention}) để thể hiện được đặc trưng thời gian trước khi đi qua lớp Cross-Local Attention.
...
%Để có thể căn chỉnh văn bản, giọng nói tương đồng với từng khung hình cử chỉ, luận văn sẽ concat các đặc trưng giọng nói, văn bản và cử chỉ đồng thời với nhau. 
%\begin{itemize}
%	\item Concat các đặc trưng âm thanh $\mathbf{A}$, văn bản $\mathbf{V}$, cử chỉ $\mathbf{X}$ và $\mathbf{Z}$ để được vector đặc trưng theo từng frame.
...
%
%Sử dụng $\mathbf{Z}$ làm $\texttt{CLS}$ token cho $\mathbf{h}^{1:M}$ trước khi đi qua Transformer Encoder.
...
\begin{equation}
	\mathbf{x}^{1:M}_{0}  = \operatorname{Transformer\ Encoder}( \operatorname{concat}( \mathbf{z}\ ||\  \mathbf{h}^{1:M}_{t}  )))
