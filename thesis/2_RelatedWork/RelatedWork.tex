...
%chứng minh mối quan hệ giữa cử chỉ và giọng nói
trình bày về các đặc điểm của cử chỉ
 \autoref{sec:relationspeechandgesture}, từ đó là tiền đề 
%thể hiện sự đồng bộ về dữ liệu giữa cử chỉ và giọng nói và 
...
%Mối quan hệ giữa cử chỉ và giọng nói
...
%$\mathcal{L} = \sum_{i=1}^N \log P(\mathbf{x}^{[1:M]} \mid \theta)$
...
%Phương pháp \textit{Speech-driven 3D Conversational Gestures from Video} \cite{habibie2021learning}, tác giả sử dụng một mạng GAN để học chuyển động của cử chỉ tương ứng với giọng nói.
...
%Graph Neural Networks for Multimodal Speech-to-Gesture Synthesis

%Cử chỉ có thể được tổng hợp bằng các mô hình xác định như perceptron đa tầng (MLP) \cite{kucherenko2020gesticulator}, 

%recurrent neural networks \cite{bhattacharya2021speech2affectivegestures}, , \cite{hasegawa2018evaluation}, \cite{yoon2020speech}, convolutional networks \cite{habibie2021learning} và transformer \cite{bhattacharya2021text2gestures} 
